...
class vardef(statebox):
    ...
    def lextok(self) -> 'lextok|None':
        if tok := self.idftok_next():
            ...
            if tok.op == '(':
                self.syntax_error("function calls cannot be targets")
            elif tok.op == '.':
                self.next()
                name = self.idftok_next(syntax=True)
...
class star_target(statebox):
    ...
    def lextok(self) -> 'lextok|None':
        if op := self.optok_next('*'):
            if self.optok('*'):
                self.syntax_error("double '*' not supported here")
...
class assign_target(statebox):
    ...
    def lextok(self) -> 'lextok|None':
        if args := list(self.gettargets()):
            ...
            if self.optok('='):
                self.syntax_error("unsupported '=' operator")
...
class simple_stmts(statebox):
    ...
    def getstmts(self):
        ...
        if tok.tabs > self.p.indent:
            self.syntax_error('unexpected indent')
...
class block(statebox):
    def lextok(self) -> 'lextok|None':
        if tok := self.tabtok_next():
            if tok.tabs <= self.p.indent:
                self.syntax_error('unexpected dedent')
